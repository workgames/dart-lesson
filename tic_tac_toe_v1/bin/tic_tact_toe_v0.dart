...
void main() {
  ...
  int min = 1;
  var rnd = new Random();
  var r = min + rnd.nextInt(3 - min);

while (state == playing) {
  int currentPlayer = r == cross ? cross : nought; // текущий игрок    
  while (true) {
  ...
  while (state == playing) {
    ...
    while (!validInput) {
      ...
      if (board[y][x] == empty) {
        ...
        if (!winFound) {
          if (List.generate(boardSize, (i) => board[i][i]).every((cell) => cell == currentPlayer)) {
            winFound = true;
          }
        }
        ...
        if (!winFound) {
          if (List.generate(boardSize, (i) => board[i][boardSize - i - 1],).every((cell) => cell == currentPlayer)) {
            winFound = true;
          }
        }
  ...
  if (state != playing) {
        ...
        buffer2.write("New game (yes, no):");
        stdout.write(buffer2.toString());
